...
// The new layout string
const topBlock = `  return (
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <main className="w-full lg:w-[70%] space-y-8">
            ...
            <nav className="flex items-center p-1.5 glass rounded-2xl w-fit flex-wrap gap-1">
              <button 
                className={\`px-6 sm:px-8 py-3 rounded-xl text-sm font-bold transition-all \${tab === "overview" ? "bg-primary text-[#0b1121]" : "text-slate-400 hover:text-white"}\`}
                onClick={() => setTab("overview")}
              >
                Overview
              </button>
              <button 
                className={\`px-6 sm:px-8 py-3 rounded-xl text-sm font-bold transition-all \${tab === "edit" ? "bg-primary text-[#0b1121]" : "text-slate-400 hover:text-white"}\`}
                onClick={() => setTab("edit")}
              >
                Edit Profile
              </button>
              <button 
                className={\`px-6 sm:px-8 py-3 rounded-xl text-sm font-bold transition-all \${tab === "password" ? "bg-primary text-[#0b1121]" : "text-slate-400 hover:text-white"}\`}
...
const editOpenStr = `              {/* ========================= EDIT PROFILE ========================= */}
              {tab === "edit" && (
                <div className="space-y-8 glass p-8 rounded-2xl">`;

const passOpenStr = `              {/* ========================= CHANGE PASSWORD ========================= */}
              {tab === "password" && (
...
// Robust find logic based on substrings instead of pure line index
let returnStart = content.indexOf('  return (\n    <div className="min-h-screen text-white">');
let editStart = content.indexOf('{tab === "edit" && (\n                <div className="space-y-8">');
let editContentStart = editStart + '{tab === "edit" && (\n                <div className="space-y-8">'.length;
let passStart = content.indexOf('{tab === "password" && (\n                <div className="space-y-6">');
let passContentStart = passStart + '{tab === "password" && (\n                <div className="space-y-6">'.length;
let passEnd = content.indexOf('</PageContainer>'); // Just below the ends

if (returnStart === -1 || editStart === -1 || passStart === -1 || passEnd === -1) {
  console.log("Could not find boundaries! Ensure file hasn't been changed.");
  process.exit(1);
}

// Extract the contents of "edit" and "password" tabs
let editSectionStr = content.substring(editContentStart, content.lastIndexOf('              {/* ========================= CHANGE PASSWORD ========================= */}'));
// Remove the trailing characters of the edit section
editSectionStr = editSectionStr.trimEnd();
if (editSectionStr.endsWith(')}')) {
  editSectionStr = editSectionStr.substring(0, editSectionStr.length - 2).trimEnd();
}
if (editSectionStr.endsWith('</div>')) {
  editSectionStr = editSectionStr.substring(0, editSectionStr.length - 6).trimEnd();
}

let passSectionStr = content.substring(passContentStart, passEnd);
passSectionStr = passSectionStr.substring(0, passSectionStr.indexOf('            </div>')).trimEnd();
if (passSectionStr.endsWith('</div>')) {
  passSectionStr = passSectionStr.substring(0, passSectionStr.length - 6).trimEnd();
}

let newFileStr = content.substring(0, returnStart) 
  + topBlock + "\\n" 
  + editOpenStr + "\\n" 
  + editSectionStr + "\\n                </div>\\n              )}\\n\\n"
  + passOpenStr + "\\n"
...
console.log("Replaced using exact indexOf boundaries successfully.");
