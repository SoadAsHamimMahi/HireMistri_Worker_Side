...
const Root = () => {
  ...
  const isAuthPage = location.pathname === '/login' || location.pathname.startsWith('/register') || location.pathname === '/registration';
  ...
  return (
    <div className={`min-h-screen app-root transition-colors duration-300 flex flex-col overflow-x-hidden ${isDashboardArea ? 'bg-[#0a0a0a]' : ''}`}>
      <Navbar />
      <main className={`flex-1 w-full ${(isDashboardArea || isAuthPage) ? '' : 'max-w-[83.333%] mx-auto'}`}>
        <Outlet />
      </main>
      <Footer />
    </div>
